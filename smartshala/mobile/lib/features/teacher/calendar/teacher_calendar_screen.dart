...
/// The teacher's read-only school calendar: a month grid with a dot per kind
/// of event, a legend that doubles as a filter, and the month's events below.
...
class _TeacherCalendarScreenState extends State<TeacherCalendarScreen> {
  ...
  Future<void> _load() async {
    final month = _month;
    try {
      final events = await context.read<TeacherRepository>().calendarMonth(month);
      // A slow response for a month the teacher has already paged away from
