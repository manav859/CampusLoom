      ...
      <div className="mt-5 min-h-0 flex-1 space-y-2 overflow-y-auto overscroll-contain pr-1">
        {loading ? (
          ...
          alerts.map((alert, index) => {
            ...
            const content = (
              <div className={`flex items-start justify-between gap-3 rounded-xl px-4 py-3 transition-colors ${toneStyles[tone]}`}>
                <div className="flex min-w-0 items-start gap-3">
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <p className="line-clamp-2 break-words text-[13px] font-bold leading-5 tracking-tight">{alert.label}</p>
                      {alert.severity ? (
                    ...
                    {alert.detail ? <p className="mt-0.5 line-clamp-2 break-words text-[12px] font-medium opacity-70">{alert.detail}</p> : null}
                  </div>
                </div>
                {alert.actionLabel && alert.onAction ? (
                  <button
                    className="shrink-0 rounded-lg bg-white/75 px-3 py-1.5 text-[11px] font-bold text-[#2456E6] shadow-sm hover:bg-white disabled:cursor-not-allowed disabled:opacity-60"
