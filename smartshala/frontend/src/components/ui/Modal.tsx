...
export function Modal({ children, description, footer, fullScreen = false, isOpen, onClose, size = "md", title }: ModalProps) {
  ...
  return (
    <div className={`fixed inset-0 z-[200] flex bg-black/40 backdrop-blur-sm ${fullScreen ? "items-stretch justify-stretch p-0" : "items-center justify-center px-3 py-4 sm:px-4"}`} role="dialog" aria-modal="true">
      <button className="absolute inset-0 cursor-default" aria-label="Close modal" onClick={onClose} type="button" />
      <div className={`relative flex w-full flex-col overflow-hidden bg-white shadow-[var(--shadow-modal)] ${fullScreen ? "h-[100dvh] max-h-[100dvh] max-w-none rounded-none" : `max-h-[calc(100dvh-2rem)] rounded-[var(--radius-xl)] ${sizeClasses[size]}`}`}>
        <button
