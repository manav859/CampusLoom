...
export function KpiCard({ label, value, helper, formula, href, tone = "neutral" }: Kpi) {
  ...
  const content = (
    <div
      className={`relative flex h-[120px] items-center gap-5 overflow-visible rounded-[6px] border border-[#E2E7EE] bg-white px-6 py-5 shadow-[0_1px_2px_rgba(15,20,25,0.06),0_8px_22px_-18px_rgba(15,20,25,0.45)] transition-colors duration-200 ${href ? "cursor-pointer" : ""} ${styles.hover}`}
    >
      <div className={`hidden h-[70px] w-[70px] shrink-0 items-center justify-center rounded-[8px] sm:flex ${styles.iconBg}`}>
        <KpiIcon label={label} className={`h-8 w-8 ${styles.accent}`} />
      </div>
      {formula ? (
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate whitespace-nowrap pr-7 text-[16px] font-medium leading-6 text-[#52687D]">{label}</p>
        <p className="mt-1 truncate whitespace-nowrap text-[30px] font-semibold leading-9 tracking-normal text-[#0F2233] [font-variant-numeric:tabular-nums]">{value}</p>
